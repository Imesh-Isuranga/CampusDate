...
export class UserService {
  private baseUrl: string = 'http://localhost:8085/user';
  ...
  userRegister(body: Student): Observable<ApiResponse> {
  ...
  getAllUsersExceptCurrentUser(): Observable<ApiResponse> {
    console.log(this.baseUrl.concat('/except/' + this.currentUser().userId));
